...
const Details = () => {
    ...
    const { name, image, brand, price, type, rating, shortDescription, updateButton } = product;
    const {User} = useContext(AuthContext);

    const handleAdd = () => {

        const email = User.email;
        const item = {...product, email}
        delete item._id;
        ...
        .then(data => {
            console.log(data);
            if(data){
                toast.success('The product Successfully added in Cart List !🤩❤️ ')
            }
        })
    }
    return (
                    ...
                    <div className="flex-1 md:p-3 lg:p-12 space-y-4">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between md:gap-5 text-sm md:text-base lg:text-lg">
                                ...
                                <button className="text-xs md:text-sm lg:text-base bg-purple-500 hover:bg-purple-600 p-3 md:px-5 lg:py-4 lg:px-9 rounded-md  text-white font-semibold cursor-pointer">Buy Now</button>
